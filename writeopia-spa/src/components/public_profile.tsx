...
export default function PublicProfile({}: Props) {
  ...
  const [connectionStatus, setConnectionsStatus] =
    useState<ConnectionsStatusTypes>();
          ...
          <Card style={{ width: "18rem" }}>
            ...
            <Card.Body>
              {userAuth.isAuthenticated &&
                connectionStatus?.sender?.userName === userAuth.username && (
                  <div className="text-center">
                    {connectionStatus?.requested === false &&
                      connectionStatus?.accepted === false && (
                        <Button
                          onClick={() => handleConnectionsStatus("add")}
                          variant="primary"
                          className="me-2"
                        >
                          Add
                    ...
                    {connectionStatus?.requested && (
                      <Button
                        onClick={() => handleConnectionsStatus("remove")}
                        variant="primary"
                        className="me-2"
                      >
                        Requested
                    ...
                    {connectionStatus?.following && (
                      <Button
                        onClick={() => handleConnectionsStatus("unfollow")}
                        variant="primary"
                      >
                        Unfollow
                      </Button>
                    )}
                    {connectionStatus?.following === false && (
                      <Button
                        onClick={() => handleConnectionsStatus("follow")}
                        variant="primary"
                      >
                        Follow
                      </Button>
