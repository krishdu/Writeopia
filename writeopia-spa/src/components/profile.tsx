  ...
  useEffect(() => {
    const getConnections = async () => {
      const response = await axios.get(
        USER_CONNECTIONS_API + userAuth.username
      );
                ...
                {connections.map((connection) => {
                  return (
                    connection.accepted === true && (
                      <>
                        <Link
                          to={`/user/${connection.sender.userName}`}
                          style={{ textDecoration: "none" }}
                          className="m-0 p-0"
                        >
                          <Card
                              ...
                              <div className="row">
                                <div className="col-3 text-center">
                                  <Card.Img
                                    ...
                                    src={`https://avatars.dicebear.com/api/avataaars/${connection.sender.userName}.svg`}
                                  />
                                </div>
