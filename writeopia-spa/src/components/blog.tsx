    ...
    if (userAuth.isAuthenticated) {
      const likeStatus = async () => {
        const response = await axios.get(
          "blogs/" + blogId + "/" + userAuth.username
        );
        console.log(response.data);
    ...
    <div>
      <div className="p-5 mb-4 bg-light rounded-3">
        <div className="container-fluid py-5 text-center">
          <Card.Img
            variant="top"
            src="https://source.unsplash.com/random/900x300/?blog,pen,write"
          />
          <h1 className="display-5 fw-bold mt-5 text-center">
            {blogdata?.title}
          </h1>
          <Link
            to={`/user/${blogdata?.myUsers.userName}`}
          ...
          <p className="col-md-8 fs-4 mt-5 mx-auto p-3">{blogdata?.body}</p>
          <button
            className="btn btn-primary btn-sm me-2"
            onClick={() => handleLikeUnlike("like")}
            disabled={
              likeStatus?.likedBy?.userName === userAuth.username ? true : false
            }
            type="button"
          >
          ...
          <button
            ...
            disabled={
              likeStatus?.unlikedBy?.userName === userAuth.username
                ? true
                : false
            }
            type="button"
          ...
          {blogdata?.publishedDate && (
            <p className="fs-10">Published on: {blogdata?.publishedDate}</p>
          )}
        </div>
      </div>
    </div>
